...
public class GetDocs extends Config{
	...
	private int subjectiveStartIndex;
	public int getSubjectiveStartIndex()
	{
		return subjectiveStartIndex;	
	}
	...
	public Seq getMap() throws CouchDbException,MappingNotCorrect{
			logger.log(Level.WARNING, "Getting Sheet Sequences DB:"+MAPDb+" HEADER: "+HEADER+" HOST: "+HOST+" PORT: "+PORT+" USR:"+USR+" PWD:"+PWD);
				...
				seqdoc.validate();
				subjectiveStartIndex=getSubjectiveStartIndex(seqdoc.getoptSeq());
	...
	public int getSubjectiveStartIndex(JsonObject array)
	{
	int counter=0;
	for( Entry<String, JsonElement> o :array.entrySet())
	{
		if(o.getValue().toString().contains("{}"))
			return counter;
		else
			counter++;
	}
